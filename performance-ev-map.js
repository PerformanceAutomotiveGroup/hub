    ...
    async function start() {
        ...
        try {
            ...
            ev_Map.addListener("idle", async () => {
                // If we are moving the map due to a marker click, SKIP the search
                if (isPanning) {
                    isPanning = false; 
                    return;
                }
    ...
    function renderUI(places, AdvancedMarkerElement) {
        ...
        places.forEach((place, index) => {
            ...
            const select = (e) => {
                ...
                // LOCK: Set panning to true so the idle event doesn't wipe the markers
                isPanning = true; 
                ...
                const infoHtml = `
                    <div style="width:320px; font-family:Roboto, Arial; background:#fff; border-radius:12px; overflow:hidden;">
                        <div style="padding:16px;"><h2 style="margin:0; font-size:20px; font-weight:400; color:#202124;">${place.displayName}</h2><div style="display:flex; gap:4px; margin:4px 0; font-size:14px;"><span>${ratingVal}</span><span style="color:#fbbc04;">★★★★★</span><span style="color:#70757a;">(1)</span></div></div>
                        <div style="display:flex; border-bottom:1px solid #e0e0e0;"><div style="flex:1; text-align:center; padding:12px; color:#00838f; border-bottom:3px solid #00838f; font-weight:500;">Overview</div><div style="flex:1; text-align:center; padding:12px; color:#70757a;">Reviews</div></div>
                        <div style="display:flex; justify-content:space-around; padding:16px; border-bottom:1px solid #f1f3f4;">
                            <div style="text-align:center; cursor:pointer;" onclick="window.open('https://www.google.com/maps/dir/?api=1&destination=${place.location.lat},${place.location.lng}')"><div style="width:40px; height:40px; border-radius:50%; background:#00838f; color:#fff; display:flex; align-items:center; justify-content:center; margin:0 auto;">↗</div><div style="font-size:11px; color:#00838f; margin-top:5px;">Directions</div></div>
                        ...
                        <div style="padding:16px; display:flex; align-items:center; gap:12px; font-size:14px; color:#3c4043;"><span style="color:#00838f; font-size:18px;">📍</span><span>${addr}</span></div>
                    </div>`;
